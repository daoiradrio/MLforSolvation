...
class ZQaQEmbeddingCond(torch.nn.Module):
    ...
    def forward(self, Z, Qa, Q, batch_seg):
        ...
        normalized = self.layer_norm(eqa)
        h = mult_Q * mult_Z * normalized + add_Q + add_Z
...
class ZQaQEmbeddingAdd(torch.nn.Module):
    ...
    def forward(self, Z, Qa, Q, batch_seg):
        ...
        eqa = self.partial_charge_embedding(Qa)
        return self.wz * ez + self.wq * eq + self.wqa * eqa, eq, 0.0
...
class ZQaQEmbeddingSimple(torch.nn.Module):
    ...
    def forward(self, Z, Qa, Q, batch_seg):
        ...
        return self.proj(h), Q_norm, 0.0



class GNN_delta_heads(GNN_GBNeck, GNN_Grapher):
    ...
    def forward(self, data):
        ...
        Bcn = torch.concat((Bc, Qa),dim=1)
        gbn_energies = self.calculate_energy(x=Bcn, edge_index=edge_index, edge_attributes=edge_attributes)
        ...
        gnn_energies_out = torch.empty((gnn_energies.size(0), 1), dtype=gnn_energies.dtype, device=gnn_energies.device)
        for qi in range(self.Q_max - self.Q_min + 1):
            mask = q_idx == qi
            gnn_energies_out[mask] = self.readout_heads[qi](gnn_energies[mask])
        
        energies = gbn_energies + gnn_energies_out
...
class GNN_delta(GNN_GBNeck, GNN_Grapher):
    ...
    def forward(self, data):
        ...
        energies = gbn_energies + gnn_energies
